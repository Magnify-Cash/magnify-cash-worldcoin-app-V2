...
const Dashboard = () => {
  ...
  const verificationLevel = nftInfo?.tier?.verificationStatus?.verification_level ?? "";
  const isOrbVerified = verificationLevel === "orb";
  const isDeviceVerified = verificationLevel === "device";
  const isVerified = ["orb", "device"].includes(verificationLevel);

  // 👇 Only using Device verification to mint Orb NFT
  const verificationLevels = {
    deviceAsOrb: {
      tierId: BigInt(2),
      level: "Orb Scan",
      icon: Globe,
      action: "upgrade-orb-verified-nft",
      ...
      try {
        ...
        if (hasActiveLoan && loan) {
          ...
          if (receivedLoans.length > 0) {
            ...
            const loanPeriodDays = loanAmount <= 1 ? 30 : loanAmount <= 5 ? 60 : 90;
  ...
  }, [ls_wallet, hasActiveLoan, loan, isVerified]);

  const handleVerify = useCallback(async (tier: typeof verificationLevels.deviceAsOrb) => {
    ...
    try {
      ...
      if (finalPayload.status === "error") {
        const errorMessage =
          finalPayload.error_code === "credential_unavailable"
            ? "You are not Device Verified in the World App. Please complete verification first."
      ...
      if (error?.message?.includes("credential_unavailable")) {
        errorMessage = "You are not Device Verified in the World App. Please complete verification first.";
      }
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-4xl mx-auto space-y-8 px-4 py-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-8 rounded-2xl shadow-lg hover:shadow-xl">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
              {Object.values(verificationLevels).map((tier) => {
                ...
                return (
                  <motion.div key={tier.level} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card p-6">
                    ...
                    <Button
                      className="w-full"
                      variant="default"
                      disabled={verifying || userTierId >= tier.tierId}
          ...
          {isVerified && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card p-8 rounded-2xl shadow-lg">
              <div className="flex items-center justify-center mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
                <motion.div key={nftInfo.tokenId} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="transform hover:scale-105 transition-transform duration-300">
                  <Card className="p-6 shadow-md hover:shadow-lg">
                    <div className="flex items-center space-x-4 mb-4">
                      <div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center">
                        <FileText className="h-6 w-6 text-primary" />
                      </div>
                      <div>
                        <h4 className="font-medium text-lg">{nftInfo?.tier.verificationStatus.level}</h4>
                      </div>
                    </div>
                    <div
                      className={`px-4 py-2 rounded-full text-sm my-3 font-medium text-center ${
                        hasActiveLoan ? "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400" : "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
                      }`}
                    >
                      {hasActiveLoan ? "Unavailable for Collateral" : "Available for Collateral"}
                    </div>
                  </Card>
                </motion.div>
              </div>
